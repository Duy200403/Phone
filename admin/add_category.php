<?php
	...
	if(isset($_POST['sbm'])){
		if(empty($_POST['cate_name'])) {
			$errors['cate_name'] = '<span style="color : red; ">Nhập tên Danh Mục</span>';
		}else{
			$cate_name = $_POST['cate_name'];
			$sqlCheck = "SELECT * FROM category WHERE cate_name = '$cate_name'";
			$queryCheck = mysqli_query($conn, $sqlCheck);
			if(mysqli_num_rows($queryCheck) > 0) {
				$errors['error'] = '<div class= "alert alert-danger">Danh Mục đã tồn tại !</div>';

			}else{
				$sqlInsertCate = "INSERT INTO category(cate_name) VALUES ('$cate_name')";
...
<body>
	...
	<div id="sidebar-collapse" class="col-sm-3 col-lg-2 sidebar">
		...
		<ul class="nav menu">
			...
			<li><a href="category.php"><svg class="glyph stroked clipboard with paper"><use xlink:href="#stroked-clipboard-with-paper"/></svg></svg>Quản lý danh mục</a></li>
